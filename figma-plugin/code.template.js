// Polaris Multi-Screen Sync Plugin (TEMPLATE)
...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'import') {
    ...
    function parseColor(cssColor) {
      ...
      return { r: parseInt(match[1]) / 255, g: parseInt(match[2]) / 255, b: parseInt(match[3]) / 255, a: match[4] ? parseFloat(match[4]) : 1 };
    }
    ...
    function parseShadows(shadowString) {
      ...
      parts.forEach(sh => {
          ...
          if (match) {
              ...
              shadows.push({
                  type: 'DROP_SHADOW', color: { r: color.r, g: color.g, b: color.b, a: color.a },
                  offset: { x: parseFloat(match[2]), y: parseFloat(match[3]) },
                  radius: parseFloat(match[4]), spread: match[5] ? parseFloat(match[5]) : 0,
